#!/usr/bin/env python3
"""
Batch scrape RST Instruments support articles - FIXED VERSION 2
- Proper title extraction (removes "Print" button text)
...
def fetch_article(url):
    ...
    # Extract title from h2 - the title is BEFORE the "Print" link
    # Pattern: <h2>Title text <link>Print</link></h2>
    h2_match = re.search(r'<h2[^>]*>(.*?)</h2>', html, re.DOTALL)
    if h2_match:
        raw_title = h2_match.group(1)
        # Remove the Print link and its surrounding elements
        title = re.sub(r'<[^>]+>', '', raw_title)  # Remove all HTML tags
        title = re.sub(r'&nbsp;', ' ', title)
        title = re.sub(r'\s+', ' ', title).strip()
        # Remove trailing "Print" that comes from the button
        title = re.sub(r'\s+Print\s*$', '', title, flags=re.IGNORECASE)
        title = title.strip()
    else:
        title = ''
